...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        Pixel[][] tgt = new Pixel[height][width];

        long totalColor = 0;
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                Pixel p = src[row][col];
                totalColor += p.getRed() + p.getGreen() + p.getBlue();
            }
        }

        int numPixels = width * height;
        int avgColor = (int) Math.round((double) totalColor / (3 * numPixels));

        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                Pixel p = src[row][col];
                int r = (int) Math.round(avgColor + multiplier * (p.getRed() - avgColor));
                int g = (int) Math.round(avgColor + multiplier * (p.getGreen() - avgColor));
                int b = (int) Math.round(avgColor + multiplier * (p.getBlue() - avgColor));
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                tgt[row][col] = getAverageColor(src, row, col, radius);
            }
        }
    ...
    // Helper function to find the average color around the pixel being blurred
    private static Pixel getAverageColor(Pixel[][] src, int row, int col, int radius) {
        int totalRed = 0;
        int totalGreen = 0;
        int totalBlue = 0;
        int count = 0;
        ...
        for (int r = row - radius; r <= row + radius; r++) {
            for (int c = col - radius; c <= col + radius; c++) {
                if (r >= 0 && r < src.length && c >= 0 && c < src[0].length) {
                    Pixel p = src[r][c];
                    totalRed += p.getRed();
        ...
        int avgRed = (int) Math.round((double) totalRed / count);
        int avgGreen = (int) Math.round((double) totalGreen / count);
        int avgBlue = (int) Math.round((double) totalBlue / count);
