...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        ...
        Pixel[][] originalPixels = pic.getBitmap();

        Pixel[][] rotatedPixels = new Pixel[width][height];

        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                rotatedPixels[width - 1 - col][row] = originalPixels[row][col];
            }
        }

        return new PixelPicture(rotatedPixels);
    }
    ...
    public static PixelPicture border(
        ...
        for (int row = 0; row < tgt.length; row++) {
            for (int col = 0; col < tgt[row].length; col++) {
                if (row < borderWidth || row >= h + borderWidth
                        || col < borderWidth || col >= w + borderWidth) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = src[row][col];
                tgt[row][col] = new Pixel(255 - p.getRed(), 255 - p.getGreen(), 255 - p.getBlue());
            }
        }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                tgt[row][col] = new Pixel(avg, avg, avg);
            }
        }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = src[row][col];
                int r = (int) Math.round(Math.min(255, p.getRed() * rfactor));
                int g = (int) Math.round(Math.min(255, p.getGreen() * gfactor));
                int b = (int) Math.round(Math.min(255, p.getBlue() * bfactor));
