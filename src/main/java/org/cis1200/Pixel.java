...
public class Pixel implements Comparable<Pixel> {
     ...
     */
    public Pixel(int[] c) {
        this(c != null && c.length > 0 ? c[0] : 0,
                c != null && c.length > 1 ? c[1] : 0,
                c != null && c.length > 2 ? c[2] : 0);
    }
    ...
    public int distance(Pixel px) {
        if (px == null) {
            ...
            int redDiff = Math.abs(this.red - px.getRed());
            int greenDiff = Math.abs(this.green - px.getGreen());
            int blueDiff = Math.abs(this.blue - px.getBlue());
    ...
    public boolean sameRGB(Pixel px) {
        if (px == null) {
            ...
            return this.red == px.getRed() && this.green == px.getGreen() && this.blue == px.getBlue();
        }
